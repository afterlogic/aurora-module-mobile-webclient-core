<template>
  <div
    class="q-gutter-y-md q-px-lg full-height flex wrap content-between login-page"
  >
    ...
    <div class="full-width q-pb-xl text-center">
      <div v-if="!isTwoFactor">
        <AppButton
          label="Login"
          :loading="loading"
      ...
      <a
        ...
        :styles="{ opacity: getAuthTokenStatus ? 0 : 1, pointerEvents: 'none' }"
      >
        Back to login
      </a>
    </div>
  ...
  methods: {
    ...
    async proceedLogin() {
      ...
      if (response && response.TwoFactorAuth) {
        ...
        this.verificationOption = response.TwoFactorAuth.HasBackupCodes
      }
    },
    verifyCode() {
